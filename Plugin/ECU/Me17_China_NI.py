...
class Me17_China_NI(Encoder):
    def __init__(self):
        super().__init__()
        self.size_min = 1572864
        self.size_max = 1572864
        
        # Последовательность байт для точного поиска
        self.search_pattern = bytes([0x82, 0x1F, 0x02, 0xF9])
        
        # Ожидаемое значение для проверки (последний байт в pattern)
        self.expected_value = 0xF9
    ...
    def find_pattern_addresses(self, buffer: bytearray) -> list:
        ...
        for i in range(len(buffer) - pattern_len + 1):
            if buffer[i:i + pattern_len] == self.search_pattern:
                # Адрес байта 0x55 в найденной последовательности (позиция 4, индекс с F9)
                target_addr = i + 3  # Исправлено: было i + 3
