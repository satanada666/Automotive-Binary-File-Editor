...
class Ems3132_No_Immo(Encoder):
    ...
    def encode(self, buffer: bytearray):
        ...
        for address in range(0x4000, 0x8000):  # До 0x8000 не включается, что правильно для 0x7FFF
            if address <= 0x7FFF:  # Ограничение до 0x7FFF
                buffer[address] = 0xFF
            else:
                break
