...
class Daewoo_Gentra_dash_denso_93c56(DashEditor):
    """Редактор для Daewoo Gentra DASH Denso 93c56."""
    ...
    def check(self, buffer: bytearray) -> bool:
        """Проверка буфера на соответствие формату EEPROM 93c56."""
    ...
    def get_mileage(self, buffer: bytearray, model: str = 'Daewoo_Gentra') -> int:
        ...
        try:
            if model == 'Daewoo_Gentra':
                locations = [
                    self.data[0x62:0x66],  # 0x62-0x65
                    self.data[0x66:0x6A],  # 0x66-0x69
                    self.data[0x6A:0x6E]   # 0x6A-0x6D
                ]
                ...
                if most_common[0][1] < 2:
                    print("get_mileage: Значения пробега не совпадают, берём последнее (0x6A-0x6D)")
                    encoded = locations[2]
                else:
    ...
    def update_mileage(self, buffer: bytearray, new_mileage: int, model: str = 'Daewoo_Gentra'):
        ...
        if model == 'Daewoo_Gentra':
            self.data[0x62:0x66] = encoded_bytes
            self.data[0x66:0x6A] = encoded_bytes
            self.data[0x6A:0x6E] = encoded_bytes
            print(f"Пробег обновлён до {new_mileage} км в позициях 0x62-0x65, 0x66-0x69, 0x6A-0x6D")
        else:
            raise ValueError("Неподдерживаемая модель")

    def encode(self, buffer: bytearray, model: str = 'Daewoo_Gentra') -> dict:
